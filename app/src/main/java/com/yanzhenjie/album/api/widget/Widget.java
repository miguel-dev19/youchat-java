...
public class Widget implements Parcelable {
    ...
    private Widget(Builder builder) {
        ...
        this.mStatusBarColor = builder.mStatusBarColor == 0 ? getColor(R.color.colorPrimaryDark) : builder.mStatusBarColor;
        this.mToolBarColor = builder.mToolBarColor == 0 ? getColor(YouChatApplication.colorTemaActual) : builder.mToolBarColor;
        this.mNavigationBarColor = builder.mNavigationBarColor == 0 ? getColor(YouChatApplication.colorTemaActual) : builder.mNavigationBarColor;
        this.mTitle = TextUtils.isEmpty(builder.mTitle) ? mContext.getString(R.string.album_title) : builder.mTitle;
        this.mMediaItemCheckSelector = builder.mMediaItemCheckSelector == null ? AlbumUtils.getColorStateList(getColor(R.color.albumSelectorNormal), getColor(YouChatApplication.colorTemaActual)) : builder.mMediaItemCheckSelector;
        this.mBucketItemCheckSelector = builder.mBucketItemCheckSelector == null ? AlbumUtils.getColorStateList(getColor(R.color.albumSelectorNormal), getColor(YouChatApplication.colorTemaActual)) : builder.mBucketItemCheckSelector;
    ...
    public static class ButtonStyle implements Parcelable {
        ...
        private ButtonStyle(Builder builder) {
            ...
            this.mButtonSelector = builder.mButtonSelector == null ?
                    AlbumUtils.getColorStateList(ContextCompat.getColor(mContext, R.color.temaVerdeAccent),
                            ContextCompat.getColor(mContext, R.color.temaRojoAccent)
                    ) :
    ...
    public static Widget getDefaultWidget(Context context) {
        return Widget.newDarkBuilder(context)
                .statusBarColor(ContextCompat.getColor(context, R.color.temaVerdeAccent))
                .toolBarColor(ContextCompat.getColor(context, R.color.temaAzulCelesteAccent))
                .navigationBarColor(ContextCompat.getColor(context, R.color.defectoAccent))
                .title(R.string.album_title)
                .mediaItemCheckSelector(ContextCompat.getColor(context, R.color.temaNaranjaAccent),
                        ContextCompat.getColor(context, R.color.temaMarronAccent))
                .bucketItemCheckSelector(ContextCompat.getColor(context, R.color.temaRosadoAccent),
                        ContextCompat.getColor(context, R.color.temaRojoAccent))
                .buttonStyle(
                        ButtonStyle.newDarkBuilder(context)
                                .setButtonSelector(ContextCompat.getColor(context, R.color.texto_negro_to_gris),
                                        ContextCompat.getColor(context, R.color.temaAzulCelesteAccent))
                                .build()
                )
                .build();
    }
