...
package cu.alexgi.youchat.chatUtils.camera;
...
import cu.alexgi.youchat.chatUtils.BitmapUtil;
import cu.alexgi.youchat.chatUtils.Optional;
import cu.alexgi.youchat.chatUtils.Prefs;
import cu.alexgi.youchat.chatUtils.Util;
import cu.alexgi.youchat.chatUtils.jobmanager.Job;
import cu.alexgi.youchat.chatUtils.jobmanager.JobParameters;
...
public class CameraView extends ViewGroup {
    ...
    enqueueTask(new SerialAsyncTask<Void>() {
      ...
      Void onRunBackground() {
        try {
          ...
          //camera=Camera.open();
          //camera = Optional.fromNullable(Camera.open());
          camera = Optional.fromNullable(Camera.open(cameraId));
          ////Log.e(TAG, "camera.open() -> " + (System.currentTimeMillis() - openStartMillis) + "ms");
          synchronized (CameraView.this) {
            CameraView.this.notifyAll();
          }
          ////Log.e(TAG, "" + camera.isPresent());
      ...
      protected void onPostMain(Void avoid) {
        if (!camera.isPresent()) {
          ////Log.e(TAG, "tried to open camera but got null");
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    //Log.e(TAG, ""+camera.isPresent()+" --- "+previewSize);
    ...
    if (previewHeight == 0 || previewWidth == 0) {
      //Log.e(TAG, "skipping layout due to zero-width/height preview size");
      return;
    }
  ...
  protected void onSizeChanged(int w, int h, int oldw, int oldh) {
    //Log.e(TAG, "onSizeChanged(" + oldw + "x" + oldh + " -> " + w + "x" + h + ")");
  ...
  /*public void setPreviewCallback233(final @NonNull PreviewCallback previewCallback) {
    ...
    new Thread(new Runnable() {
      ...
      public void run() {
        ...
        try {
          ...
          //Log.e(TAG, "Catch");
          String cad=" +++ "+e;
          //Log.e(TAG, "couldn't set preview display"+cad);
        }
        //return null;
      }
    }).run();
  ...
  private void startPreview(final @NonNull Parameters parameters) {
    //Log.e(TAG, "CAMARA-START_PREVIEW"+this.camera.isPresent() );
    if (this.camera.isPresent()) {
      try {
        ...
        //Log.e(TAG, ""+preferredPreviewSize +" *-*-*-* "+parameters.getPreviewSize().equals(preferredPreviewSize));
        if (preferredPreviewSize != null && !parameters.getPreviewSize().equals(preferredPreviewSize)) {
          //Log.e(TAG, "starting preview with size " + preferredPreviewSize.width + "x" + preferredPreviewSize.height);
        ...
        camera.startPreview();
        //Log.e(TAG, "camera.startPreview() -> " + (System.currentTimeMillis() - previewStartMillis) + "ms");
        state = State.ACTIVE;
  ...
  public void takePicture(final Rect previewRect) {
    if (!camera.isPresent() || camera.get().getParameters() == null) {
      //Log.e(TAG, "camera not in capture-ready state");
      return;
    }
    ...
    camera.get().setOneShotPreviewCallback(new Camera.PreviewCallback() {
      ...
      public void onPreviewFrame(byte[] data, final Camera camera) {
        ...
        //Log.e(TAG, "data bytes: " + data.length);
        //Log.e(TAG, "previewFormat: " + camera.getParameters().getPreviewFormat());
        //Log.e(TAG, "croppingRect: " + croppingRect.toString());
  ...
  private void enqueueTask(SerialAsyncTask job) {
    YouChatApplication.getInstance(getContext()).getJobManager().add(job);
  }
  ...
  private static abstract class SerialAsyncTask<Result> extends Job {
    ...
    @Override public final void onRun() {
      try {
        ...
        //Log.e(TAG, "skipping task, preconditions not met in onWait()");
      }
    }
    ...
    @Override protected void onWait() throws PreconditionsNotMetException {
      synchronized (CameraView.this) {
        ...
        while (getMeasuredHeight() <= 0 || getMeasuredWidth() <= 0 || !surface.isReady()) {
          //Log.e(TAG, String.format("waiting. surface ready? %s", surface.isReady()));
