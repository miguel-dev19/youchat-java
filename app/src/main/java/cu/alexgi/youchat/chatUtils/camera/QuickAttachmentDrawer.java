...
import cu.alexgi.youchat.chatUtils.ServiceUtil;
...
public class QuickAttachmentDrawer extends ViewGroup implements InputAwareLayout.InputView, CameraView.CameraViewListener {
  ...
  private void updateControlsView() {
    ...
    View controls = LayoutInflater.from(getContext()).inflate(isLandscape() ? R.layout.quick_camera_controls_land
                                                              ...
                                                              this, false);
    shutterButton    = controls.findViewById(R.id.shutter_button);
    swapCameraButton = controls.findViewById(R.id.swap_camera_button);
    ...
    if (cameraView.isMultiCamera()) {
      ...
      swapCameraButton.setImageResource(cameraView.isRearCamera() ? R.drawable.camera_revert1
  ...
  private void setDrawerState(DrawerState drawerState) {
    switch (drawerState) {
    case COLLAPSED:
      fullScreenButton.setImageResource(R.drawable.camera_expand_outline);
      break;
    case HALF_EXPANDED:
      if (isLandscape()) {
      ...
      break;
    case FULL_EXPANDED:
      fullScreenButton.setImageResource(isLandscape() ? R.drawable.fab_chat_down
                                                      : R.drawable.camera_collapse_outline);
      break;
    }
